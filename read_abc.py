...
import pysynth
measure_sharps_flats = 0
def to_wav(fn):
    global measure_sharps_flats, triplet, tie_next, song, only_first, second_ver, chord
    song = []
    ...
    key_sigs = (
        ...
        ("A",    "F#m",   3),
        ("EMix", "BDor",  3),
        ("Eb",   "Cm",   -3),         # Suspect that more than this awill never be
    ...
    def get_bpm(s, u = "1/4"):
        if '=' not in s:
            c, d = u.split('/')
            return int(s) * 8. * float(c) / float(d)
        else:
            a, b = s.split('=')
            c, d = a.split('/')
            return int(b) * 8. * float(c) / float(d)
    f = open(fn)
